...
def inch_swap(privatekey, amount_to_swap, to_token_address, to_symbol):
    try:
        ...
        web3 = Web3(Web3.HTTPProvider(config.RPC, request_kwargs=config.request_kwargs))
...
def inch_swap_sell(privatekey, amount_to_swap, fromTokenAddress, to_symbol):
    try:
        ...
        if to_symbol == 'DBL':
            ...
            need = round(random.uniform(0.015, 0.02), 6)
        if to_symbol == 'LPT':
            ...
            need = 0.0012
            need = round(random.uniform(0.0015, 0.002), 6)
        ...
        need_amount = intToDecimal(need, 18) 
...
swaps_1inch = [
    ...
    'symbol': 'DBL',
    'amount': round(random.uniform(0.001, 0.00101), 8)},
...
if __name__ == "__main__":
    ...
    cprint(f'\n============================================= hodlmod.eth И Плюшкин =============================================', 'cyan')
    cprint(f'\nsubscribe to us : https://t.me/plushkin_blog', 'green')
    ...
    for privatekey in keys_list:
        ...
        cprint(f'\n=============== start sleep ===============', 'white')
        time.sleep(random.randint(config.timeoutMin, config.timeoutMax))
        requests.get(url=config.changeIPlink)
